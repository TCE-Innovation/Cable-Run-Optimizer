...
def obtain_cable_data():
    ...
    for file_name in os.listdir(folder_path):
        ...
        if file_name.endswith('.xlsx') or file_name.endswith('.xls'):
            ...
            print("Pull Column Index:", pull_col_index)
            ...
            # Create list to store all cables from pull sheet
            cable_list = []

            # Iterate over the rows to extract information from relevant columns
            for row in sheet.iter_rows(min_row=2):
                pull_number = sheet.cell(row=row[0].row, column=pull_col_index).value if pull_col_index != -1 else None
                stationing_start = sheet.cell(row=row[0].row,
                                              column=start_col_index).value if start_col_index != -1 else None
                stationing_end = sheet.cell(row=row[0].row, column=end_col_index).value if end_col_index != -1 else None
                cable_size = sheet.cell(row=row[0].row, column=size_col_index).value if size_col_index != -1 else None
                express = sheet.cell(row=row[0].row,
                                     column=express_col_index).value if express_col_index != -1 else None

                cable = Cable(pull_number, stationing_start, stationing_end, is_express, cable_size)
                cable_list.append(cable)
            '''
            for row in sheet.iter_rows(min_row=2, values_only=True):
                pull_number = row[0]
                stationing_start = row[1]
                stationing_end = row[2]
                cable_size = row[3]
                is_express = row[4]
            '''
    ...
    for cable in cable_list:
        ...
        print("Cable Size:", cable.is_express)
