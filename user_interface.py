...
class CableSizingWindow(QWidget):
    def __init__(self):
        ...
        self.cable_sizes_table = QTableWidget()
        layout.addWidget(self.cable_sizes_table)
        ...
        self.update_cable_sizes()

    def update_cable_sizes(self):
        # Set the number of rows and columns in the table
        num_rows = len(cable_sizes)
        num_columns = 4  # Assuming cable parameters have four attributes: size, diameter, cable weight, and cross-sectional area

        self.cable_sizes_table.setRowCount(num_rows)
        self.cable_sizes_table.setColumnCount(num_columns)

        # Set the table headers
        headers = ["Size", "Diameter", "Cable Weight", "Area"]
        self.cable_sizes_table.setHorizontalHeaderLabels(headers)

        # Initialize the area_labels list
        self.area_labels = []

        # Populate the table with the cable_sizes:
        for row, cable in enumerate(cable_sizes):
            ...
            self.cable_sizes_table.setCellWidget(row, 0, size_label)
            ...
            self.cable_sizes_table.setCellWidget(row, 1, diameter_spinbox)
            ...
            self.cable_sizes_table.setCellWidget(row, 2, cable_weight_spinbox)
            ...
            self.cable_sizes_table.setCellWidget(row, 3, cross_sectional_area_label)
        ...
        for column in range(num_columns):
            self.cable_sizes_table.setColumnWidth(column, 95)

        # Resize the rows to fit the contents
        self.cable_sizes_table.resizeRowsToContents()
